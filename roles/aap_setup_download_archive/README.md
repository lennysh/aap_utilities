...
* `aap_setup_down_checksum_cache_enabled` — when `true` (default), store each file’s SHA-256, size, API `datePublished`, and `imageName` (as `image_name`) in a JSON file so later runs skip the expensive checksum `stat` when the on-disk size still matches the cache (metadata-only `stat` still runs per file). Set `false` to always compute SHA-256 on disk.
